...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=30,
        type=int,
        help="Number of k-nearest-neighbor to consider.",
    )
...
def main():
    ...
    retdict["four_atom_coords"] = torch.from_numpy(four_atom_coords).float()
    ...
    node_angle_feat = np.concatenate(
        [np.cos(node_angle_feat), np.sin(node_angle_feat)], axis=1
    )
    ...
    for atom_idx in [N_IDX, C_IDX, O_IDX]:
        ...
        vec_to_ca = vec_to_ca / np.linalg.norm(
            vec_to_ca, axis=1, keepdims=True
        )  # Normalize
